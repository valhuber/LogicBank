...
def copy_gold_over_db():
    ...
    print("\n********************************\n"
          "  IMPORTANT - copy_gold_over_db: copy db/database.db from db/database-gold.db in " + basedir + "/passive_delete_cascade/db/\n" +
...
def new_session_from_gold():
    ...
    Enables SQLite's per-connection FK pragma (required for passive_deletes=True /
    ON DELETE CASCADE to actually take effect - SQLite ignores FK constraints,
    including cascades, unless this is set on every connection).
