...
def copy_gold_over_db():
    ...
    print("\n********************************\n"
          "  IMPORTANT - copy_gold_over_db: copy db/database.db from db/database-gold.db in " + basedir + "/multi_relns/db/\n" +
...
def new_session_from_gold():
    ...
    Unlike examples/nw (one DB-mutating test method per file, so the module-level
    session/engine singleton is only ever used once), this suite's test files have
    multiple mutating test methods per class. copy_gold_over_db() alone only resets
    the underlying SQLITE FILE - an already-open session/engine retains its own
    connection/identity-map state, so file resets are invisible to it. Each test
    method must get a genuinely fresh engine+session+activate() cycle, not just a
    fresh file underneath a stale connection.
