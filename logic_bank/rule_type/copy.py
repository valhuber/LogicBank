...
class Copy(Derivation):
    ...
    def __init__(self, derive: InstrumentedAttribute, from_parent: any, child_role_name: str = ""):
        ...
        if isinstance(from_parent, str):
            ...
            if child_role_name is not None and child_role_name != "":
                self._from_parent_role = child_role_name  # honor explicit disambiguator (mirrors Count)
            else:
                    ...
                    if isinstance(each_attr, RelationshipProperty):
                        ...
                        if each_parent_class_name == self._parent_class_name:
                            if found_attr is not None:
                                # NB: cannot use {self} here - __str__() reads self._from_parent_role,
                                # which isn't set yet (that's the very ambiguity being reported)
                                raise Exception("Ambiguous Relationship - add 'child_role_name' for "
                                                f"{self.get_derived_attribute_name()}")
