...
This guide covers testing LogicBank using `run_tests.py`.

> **AI assistants:** see [system/LogicBank-Internal-Dev/dev-architecture.md](system/LogicBank-Internal-Dev/dev-architecture.md) for architecture and design context; [system/LogicBank-Internal-Dev/release-management.md](system/LogicBank-Internal-Dev/release-management.md) for the release/build/PyPI process (split out from this file).
...
#### Test Organization
- **9 Example Directories:** Each with focused test scenarios
- **Mix of unittest and standalone scripts**
- **Test Types:**
  - **Unittest files** (NW, multi_relns): Uses Python unittest framework
...
9. **Multi-Relations:** Multi-relationship-to-same-parent regression suite (Sum/Count/Copy/Formula-cascade/link disambiguation)

### Debugging Workflow

#### When Tests Pass ✅
- Review `test_summary.txt` for confirmation
- All test suites should show `PASSED`
...
5. **Modernized build system**: Migrated from setup.py to pyproject.toml (see `system/LogicBank-Internal-Dev/release-management.md`)

This development approach ensures comprehensive validation while enabling efficient debugging of specific issues.
